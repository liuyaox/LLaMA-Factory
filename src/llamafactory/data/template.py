...
class Template:
    ...
    def _encode(
        ...
        for i, message in enumerate(messages):      # YAO: message是一句对话，messages是一个对话session
            ...
            if i > 0 and i % 2 == 0:    # YAO：序号2,4,6等偶数句子(每轮开始)前拼接的是separator(轮次分隔符，很多模型其实是空字符串)
...
def _register_template(
    ...
    TEMPLATES[name] = template_class(
        ...
        format_separator=format_separator or default_separator_formatter,   # YAO：若没指定，使用默认的
...
def get_template_and_fix_tokenizer(
    ...
    stop_words = template.stop_words    # YAO: stop_words作用：一作为eos_token(若有要求)；二作为special_token添加到vocab中
