...
def _encode_supervised_example(
    ...
    messages = prompt + response    # YAO: prompt和response组装为一个完整对话session  详见src/llamafactory/data/aligner.py
    input_ids, labels = [], []
    ...
    for turn_idx, (source_ids, target_ids) in enumerate(encoded_pairs):     # YAO：一轮一轮处理，然后拼接各轮对话，所以不用再提前手动拼接了！
        ...
        if train_on_prompt:             # YAO: train_on_prompt是说，训练时，既学prompt，也学response TODO 应用场景是？
